...
use JULIET\api\JULIET as JULIET;
...
Phpbb::make_phpbb_env();  
    $mysqli = db::get_mysqli();//mysqli_set_charset($mysqli, "utf8");
    if(Rights::user_can("USER_IS_SIBYLLA")) {
        $name = $mysqli->real_escape_string($name);
        switch($cat) {
            case "tag" :
                // Si le tag qu'on cherche est un TAG banal.
                $tags = $mysqli->query('SELECT * FROM star_tags WHERE name="'.$name.'"');
                ...
                while($player = $players->fetch_assoc()) {
                    ...
                    $player['avatar'] = JULIET::ju_get_avatar($player['id_forum']);
                    $player['name'] = JULIET::get_names($player['id_forum']);
        ...
        foreach($allShips as $ship) {
            ...
            while($player = $players->fetch_assoc()) {
                if($player['id_forum'] > 0) {
                    ...
                    $player['name'] = utf8_encode(JULIET::get_names($player['id_forum']));
            ...
            if($all) {
                ...
                while($s = $shipsB->fetch_assoc()) {
                    ...
                    $s['owner_name'] = utf8_encode(JULIET::get_names($s['owner']));
...
case "SPECIAL":
    ...
    switch($name)	{
        ...
        foreach($return['count'] as $id) {
            ...
            $player['name'] = utf8_encode(JULIET::get_names($id));
            $player['id_forum'] = $id;
            ...
            if($all) {
                ...
                while($ship = $query->fetch_assoc()) {
                    ...
                    $ship['owner_name'] = utf8_encode(JULIET::get_names($ship['owner']));
